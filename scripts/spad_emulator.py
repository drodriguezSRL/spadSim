...
PHOTONS_PER_PX = (
    1000  # number of photons per pixel per RGB exposure at normalized intensity = 1
)
...
def load_image_gray(p: str) -> np.ndarray:
    ...
    )  # converts from PIL image object to numpy array. float32 is needed for optical flow.


def compute_farneback(img0_gray: np.ndarray, img1_gray: np.ndarray) -> np.ndarray:
    """
    ...
    flow = cv2.calcOpticalFlowFarneback(
        ...
        poly_n=5,
        poly_sigma=1.2,  # polynomial expansion parameters
...
def warp_image(img_gray: np.ndarray, flow: np.ndarray, alpha: float) -> np.ndarray:
    """
    ...
    dx = (alpha * flow[..., 0]).astype(
        np.float32
    )  # flow[..., 0] = horizontal pixel movement (dx)
    dy = (alpha * flow[..., 1]).astype(
        np.float32
    )  # flow[..., 1] = vertical pixel movement (dy)
...
def extract_frames_from_video(
    ...
    - target_fps (Optional[float]): Desired frames per second for extraction. If None, uses video's native FPS.
    - max_frames (Optional[int]): Optional maximum number of frames to extract.

    Returns:
    - A tuple containing: number of extracted frames, list of file paths, detected FPS, and fps_source ("auto-detected" or "user-provided").
    """
    # open video file
    try:
    ...
    # determine target FPS and fps_source
    if target_fps is None:
        target_fps = orig_fps
        fps_source = "auto-detected"
    else:
        fps_source = "user-provided"
    ...
    while True:
        ...
        if frame_idx / max(1, step) - extracted >= 1 - EPSILON:
            ...
            Image.fromarray(rgb).save(
                out_path
            )  # directly saving with cv2.imwrite(str(out_path), frame) may be faster for high-volume extraction [to check]
    ...
    # returns number of extracted frames, their paths, detected FPS, and fps_source
    return extracted, out_paths, target_fps, fps_source


def load_frames_from_directory(
    dir_path: str, max_frames: Optional[int] = None
) -> Tuple[int, list]:
    """
    ...
    if not dir_path.is_dir():
        raise RuntimeError(
            f"Input directory {dir_path} does not exist or is not a directory."
        )
...
def generate_spad_sequence_from_pair(
    ...
    for j in range(N):
        ...
        lambda_signal = (rgb_photons * i_norm * QE * (t_spad / t_rgb)).astype(
            np.float64
        )
        ...
        sum_lambda_dark += float(
            lambda_dark
        )  # same for all pixels if constant dark_rate
...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate binary SPAD frames from an RGB video, directory of RGB images, or single RGB image."
    )
    parser.add_argument(
        "input",
        type=str,
        help="Path to the input RGB video file, directory containing RGB images, or single RGB image file.",
    )
    parser.add_argument(
        "--output_dir",
        "-o",
        type=str,
        default="/output_dir",
        help="Path to the output directory for frames and metadata.",
    )
    parser.add_argument(
        "--rgb_fps",
        "-f",
        type=float,
        default=None,
        help="Target FPS for RGB frame extraction (Hz). If not provided, uses video's native FPS (for videos). For image directories, defaults to 30 Hz.",
    )
    parser.add_argument(
        "--max_frames",
        "-m",
        type=int,
        default=None,
        help="Maximum number of RGB frames to extract/process (for testing).",
    )
    parser.add_argument(
        "--spad_rate", "-sf", type=float, default=SPAD_FPS, help="SPAD frame rate (Hz)"
    )
    parser.add_argument(
        "--rgb_photons",
        "-p",
        type=float,
        default=PHOTONS_PER_PX,
        help="Number of photons per pixel for one RGB frame when the normalized pixel intensity is maximum (i = 1).",
    )
    parser.add_argument(
        "--quantum_efficiency",
        "-qe",
    ...
    parser.add_argument(
        ...
        default=INCLUDE_DCR,
        choices=[True, False],
        help="Include dark counts (True/False)",
    )
    parser.add_argument(
        "--dcr",
        "-d",
        type=float,
        default=SPAD_DCR,
        help="Dark count rate per pixel (counts/s)",
    )
    parser.add_argument(
        "--detection_threshold",
        "-dt",
        ...
        help="Detection threshold (int >=1)",
    )
    parser.add_argument(
        "--optical_flow_method",
        "-ofm",
        type=str,
        default=OPTFLOW_METHOD,
        choices=["farneback"],
    ...
    parser.add_argument(
        ...
        default=SAVE_RGB,
        choices=[True, False],
        help="Save extracted RGB frames (True/False)",
    )
    parser.add_argument(
        "--seed", type=int, default=SEED, help="Random seed (0 means random)"
    )
    parser.add_argument(
        "--duration",
        type=float,
        default=DURATION,
        help="Duration in seconds for single image input (default: 1.0)",
    )
    ...
    fps_source = None  # will be set based on detection method

    if input_path.is_file():
        if input_path.suffix.lower() in video_extensions:
            # Input is a video file
            print("⏳Extracting RGB frames from video...")
            num_frames, rgb_paths, detected_fps, fps_source = extract_frames_from_video(
                video_path=str(input_path),
                out_dir=rgb_dir,
                target_fps=args.rgb_fps,  # None if not provided, otherwise user-provided value
                max_frames=args.max_frames,
            )
            # If user didn't provide fps, use detected fps
            rgb_fps = detected_fps
            input_type = "video"
            ...
            rgb_fps = DEFAULT_FPS  # default for single images
            fps_source = "default"
            if args.save_rgb:
        ...
        num_frames, rgb_paths = load_frames_from_directory(
            dir_path=str(input_path), max_frames=args.max_frames
        )
        # For directory, use provided fps or default
        rgb_fps = args.rgb_fps if args.rgb_fps is not None else DEFAULT_FPS
        fps_source = "user-provided" if args.rgb_fps is not None else "default"
        if args.save_rgb:
    ...
    if input_type != "single_image" and num_frames < 2:
        raise RuntimeError(
            f"At least 2 RGB frames are required to generate interpolated SPAD frames."
        )
    ...
    if input_type == "single_image":
        ...
        n_spad_per_pair = int(
            round(t_rgb / t_spad)
        )  # number of SPAD frames pair RGB pair (approx)
    ...
    metadata = {
        ...
        "fps_source": fps_source if input_type != "single_image" else None,
        "spad_rate": spad_fps,
    ...
    if len(all_diagnostics) > 0:
        mean_lambda_signal = float(
            np.mean([d["mean_lambda_signal"] for d in all_diagnostics])
        )
        mean_lambda_dark = float(
            np.mean([d["mean_lambda_dark"] for d in all_diagnostics])
        )
        mean_lambda_total = float(
            np.mean([d["mean_lambda_total"] for d in all_diagnostics])
        )
        mean_detection_prob = float(
            np.mean([d["mean_detection_prob_empirical"] for d in all_diagnostics])
        )
    else:
        mean_lambda_signal = mean_lambda_dark = mean_lambda_total = (
            mean_detection_prob
        ) = 0.0
    summary["aggregate"] = {
