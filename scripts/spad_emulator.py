...
def extract_frames_from_video(
    video_path: str, out_dir: str, target_fps: float, max_frames: Optional[int] = None
) -> Tuple[int, list]:
    """
...
def create_video_from_frames(
    frames_dir: str, output_video_path: str, fps: float = 30.0
) -> None:
    """
    Create an MP4 video from SPAD frames (PNG images with numerical naming).

    Parameters:
    - frames_dir (str): Directory containing the SPAD frame images (e.g., spad_frames/)
    - output_video_path (str): Path for the output MP4 video file
    ...
    if not frames_dir.is_dir():
        raise RuntimeError(
            f"Frames directory {frames_dir} does not exist or is not a directory."
        )
    ...
    if not frame_files:
        raise RuntimeError(f"No PNG frames found in directory {frames_dir}.")
    ...
    if not out.isOpened():
        raise RuntimeError(
            f"Could not open VideoWriter for output: {output_video_path}"
        )

    # Write frames to video
    print(f"📹 Creating video from {len(frame_files)} frames...")
    for frame_path in tqdm(frame_files, desc="Writing frames to video"):
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=DEFAULT_FPS,
        help="Target FPS for RGB frame extraction (Hz). For image directories, this sets the assumed frame rate.",
    )
    parser.add_argument(
        "--max_frames",
        "-m",
    ...
    parser.add_argument(
        ...
        "-sv",
        action="store_true",
        help="Save SPAD frames as MP4 video (True/False)",
    )
    parser.add_argument(
        "--spad_video_dir",
        "-svd",
        type=str,
        default="./output_dir",
        help="Output directory for SPAD MP4 video",
    )
    ...
    # Create video from SPAD frames if requested
    if bool(args.save_spad_video):
        spad_video_output = Path(args.spad_video_dir) / "spad_frames.mp4"
        try:
            create_video_from_frames(
                frames_dir=str(spad_dir),
                output_video_path=str(spad_video_output),
                fps=spad_fps,
            )
